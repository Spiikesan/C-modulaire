...
void		opt_del(t_object *ptr)
{
	...
	if (!ptr)
		return ;
	g = (t_getopt *)ptr;
	FOREACH(t_opt_v *, o, g->opts, free(o));
	delete(g->opts);
	free(g->optstr);
}

CMETA_STRUCT_BUILD(t_getopt_DEFINITION)
{
	t_getopt	*g = newObject(t_getopt, &opt_del);
...
int		opt_register(t_getopt *opt, char c, t_opt_type type, t_opt_par par)
{
	...
	if (!opt || !isgraph(c) || type > N_OPT_TYPES)
		return (OPT_PARAMS);
...
int	opt_parse_params(t_getopt *opt, int ac, char **av)
{
	...
	if (!opt || ac <= 1 || !av)
		return (OPT_PARAMS);
	FOREACH(t_opt_v *, o, opt->opts, o->set = 0)
